...
def load_data():
    ...
    # Defensive fallback: if this daily_activity table came from an older/
    # different fitness.db that doesn't have the merged sleep/weight columns,
    # add them as empty so the app degrades gracefully instead of crashing.
    for col, default in [
        ("HasSleepData", False), ("HasWeightData", False),
        ("MinutesAsleep", pd.NA), ("MinutesInBed", pd.NA),
        ("WeightKg", pd.NA), ("BMI", pd.NA), ("IsManualReport", pd.NA),
    ]:
        if col not in daily.columns:
            daily[col] = default
    daily["HasSleepData"] = daily["HasSleepData"].fillna(False).astype(bool)
    daily["HasWeightData"] = daily["HasWeightData"].fillna(False).astype(bool)
